...
// Throws UnionCompatibilityException if r and s don't have the same number of columns.
...
// Return a copy of the input table with some or all of the columns
// renamed according to the name_map.  E.g. if the input has columns
// (a, b), then a name_map of {{"a", "x"}} will yield a table with
// columns named (x, b).  Throws TableException if the name_map tries
// to rename columns that don't exist, or renames a column more than
// once.
