                   ...
                   int expected_sum_balances)
{
    ...
    for (int t = 0; t < transactions_per_user; t++) {
        ...
        if (outcome.status() == AUDIT_FAILED) {
            failed_audit++;
        } else if (outcome.amount() != expected_sum_balances) {
            bad_audit++;
        }
    }
    disconnect_from_database(connection);
                          ...
                          int transactions_per_user)
{
    ...
    int expected_sum_balances;
    {
        ...
        expected_sum_balances = audit(connection).amount();
        disconnect_from_database(connection);
    }
